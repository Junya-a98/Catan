...
- 部屋作成、6文字の参加コード、2〜4人、勝利点、盤面mode / seed、通常 / 予告イベントmode
...
予告イベントmodeでは、次のイベント名、発動までの残り手番、現在有効な効果を対局画面の専用カードへ表示します。未来のイベント順はbrowserへ送らず、権威サーバーが予告済みイベントだけを公開します。最初のcatalogは、次の麦生産へ条件付きボーナスを付ける「豊作」と、1ラウンドだけ羊生産を止める「大干ばつ」です。
...
予告イベントmodeのAIは、公開中の次回イベントと現在有効な効果だけを評価します。たとえば大干ばつ中は羊の期待を下げ、豊作の予告中は麦の価値を少し上げます。権威サーバーだけが保持する未来のevent deckは参照しません。
...
1. 標準19タイル内を霧で隠すフロンティア探索modeを追加
2. ブラウザへカスタムマップとハウスルール設定を追加
3. room / session / replay storeをprocess外へ分離し、server再起動後も復帰可能にする
